...
class HuggingFaceGraph(GraphDataset):
    """
    Parent class HuggingFace graph datasets. To use this class, 
    the underlying HuggingFace dataset must have a text and a label attribute.
    """
    def __init__(self, device, val_size=0.1, n_train_docs=None, dataset_name="ag_news"):
        """
        Creates the train, test, and validation splits.
        Args:
        ...
        """
        super(HuggingFaceGraph, self).__init__(device, n_train_docs)

        docs, labels, classes = self.prepare(val_size, n_train_docs, dataset_name)
        ...
        super(HuggingFaceGraph, self).initialize_data(docs, classes)

    @staticmethod
    def prepare(val_size=0.1, n_train_docs=None, dataset_name="ag_news"):
        """
        Return the training, validation, and tests splits along with the classes of the dataset.
        Args:
            val_size (float, optional): Proportion of training documents to include in the validation set.
            n_train_docs (int, optional): Number of documents to use from the training set. If None, include all.
        Returns:
